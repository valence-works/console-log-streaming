...
/// </summary>
public sealed class ConsoleLogSubscriptionManager : IDisposable
{
    ...
    private async Task StreamAsync(string connectionId, ConsoleLogFilter filter, ConsoleLogSubscription subscription, CancellationToken cancellationToken)
    {
        try
        {
            await foreach (var item in _provider.SubscribeAsync(_mapper.ToCore(filter), cancellationToken).ConfigureAwait(false))
            {
                if (item.Line != null)
    ...
    private static bool MatchesSource(CoreConsoleLogSource source, ConsoleLogFilter filter)
    {
