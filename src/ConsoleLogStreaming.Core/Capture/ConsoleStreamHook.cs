...
public static class ConsoleStreamHook
{
    ...
    public static void Install()
    {
        ...
        lock (Lock)
        {
            ...
            Console.SetOut(new ConsoleStreamTeeWriter(_originalOut, ConsoleStream.Stdout, Dispatch));
            Console.SetError(new ConsoleStreamTeeWriter(_originalError, ConsoleStream.Stderr, Dispatch));
            _installed = true;
        }
    }
    ...
    public static void Uninstall()
    {
        lock (Lock)
        {
            ...
            _subscriptions = [];
            RestoreOriginalWriters();
        }
    }

    internal static void UninstallIfIdle()
    {
        lock (Lock)
        {
            if (!_installed || _subscriptions.Length != 0)
                return;
    ...
    private static void Dispatch(ConsoleStream stream, string value)
    {
    ...
    private static void RestoreOriginalWriters()
    {
...
/// </summary>
public readonly record struct CapturedChunk(ConsoleStream Stream, string Text, DateTimeOffset TimestampUtc);
